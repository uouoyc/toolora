...
- `font-sans` = `--font-sans`，Geist Mono 通过 CSS 变量提供；mono 在 UI 中承担"微标签"角色（见 §3 排版标尺）。
...
| 微标签        | ` text-[10px] tracking-widest uppercase` | 10     | 400/500                      | +0.1em   | 表头、指标标签、RUN HINT、徽章、胶囊 |
...
<div className="space-y-2">
  ...
  <input
    className="… mt-2 h-12 w-full rounded-xl border border-border
...
| 抽屉 Key textarea     | `rows=6`                                | **18** | `px-5 py-4` + ` text-sm` | `ring-primary/10`（更淡）                   |
...
<div className="flex flex-col gap-4 md:flex-row md:items-stretch">
  <div className="bg-muted/50 border-border flex-1 rounded-xl border p-4">   {/* 提示盒 */}
    <p className="…  text-[10px] tracking-widest uppercase">RUN HINT</p>
    <p className="text-muted-foreground text-xs">…规则说明…</p>
    <p className="mt-2  text-xs">预计 SerpAPI 尝试次数：36</p>     {/* 数字用 mono */}
  </div>
...
<div className="rounded-2xl border border-green-500/20 bg-green-500/10 p-4 text-sm font-medium
  ...
  <span className="mr-2  font-bold">STATUS / SUCCESS</span> 查询完成。…
```
...
<div className="grid w-full grid-cols-2 gap-4 lg:grid-cols-4">
  <div className="border-border bg-card flex flex-col rounded-3xl border p-6">
    <p className="text-muted-foreground mb-2  text-[10px] tracking-widest uppercase">已找到</p>
...
| `<th>`        | `px-4 py-4  text-[10px] tracking-widest uppercase text-muted-foreground`                    |
| 关键词列      | `px-4 py-4 font-medium`（16px medium）                                                      |
| 排名列        | `px-4 py-4 `（16px mono）                                                                   |
| URL 列        | `max-w-50 truncate px-4 py-4 text-sm text-muted-foreground`（200px 截断）                   |
| Key 列        | ` text-xs text-muted-foreground`（**我们不采用**）                                          |
...
- Key 健康表：容器 `rounded-2xl border bg-card/50`，表头 `bg-muted/30 border-b` + 微标签配方，数据列 ` text-xs`，行 `hover:bg-accent/20`。
...
4. **mono 微标签母题**：` text-[10px] tracking-widest uppercase` 贯穿表头、指标标签、徽章、胶囊计数、RUN HINT——是全站最强的"工程感"签名。
...
| 微标签         | ` text-[10px] tracking-widest uppercase text-muted-foreground`                                                                                          |
...
| 表头格         | `px-4 py-4  text-[10px] tracking-widest uppercase text-muted-foreground`                                                                                |
